...
exports.setCreateComment = (req,res) => {
    ...
    if(content_id){
        content_db.create_comment(comment_text, req.session.user.user_id, content_id, comment_id,(err) => {
            if (err){
        ...
        content_db.re_create_comment(comment_text, comment_id, req.session.user.user_id,(err, result) => {
            if (err){
                console.log("error code ", err);
...
exports.setInvisiblyctl = (req, res) => {
    ...
    if(content_id){
        content_db.set_invisibly_content(content_id, req.session.user.user_id , (err, result) => {
            if(err){
        ...
        content_db.set_invisibly_comment(comment_id, req.session.user.user_id,(err,result) => {
            if(err){
                return res.status(500).json({message : "삭제 처리를 하는 도중 에러가 발생했습니다."});
            }
